...
def main() -> None:
    ...
    out_npz = out_dir / f"d1_d3_d5_d7_capacity_vs_sr_N{args.num_nodes}_al{args.alpha:.3f}_om{args.omega:.3f}.npz"
    ...
    if out_npz.exists():
        ...
        for sr in sr_values:
            ...
            cap_iter = capacity_calculation.CapacityIterator(mindeg=1, maxdeg=5, m_degrees=True)
            ...
            print(f"sr={sr:.3f}: d1={d1_score:.3f}, d3={d3_score:.3f}, d5={d5_score:.3f}, d7={d7_score:.3f} (elapsed {elapsed:.2f}s)")
        ...
        np.savez(out_npz, sr_values=sr_values, d1_scores=d1_scores, d3_scores=d3_scores, d5_scores=d5_scores,
    ...
    plt.plot(sr_values, d5_scores, 'd:', label='degree 5')
    plt.plot(sr_values, d7_scores, '^-', label='degree 7')

    plt.xlabel('Spectral radius')
    plt.ylabel('Capacity')
    plt.title('Degree-1, Degree-3, Degree-5, and Degree-7 capacity vs spectral radius')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
