...
class ESN:
    """
    Echo State Network (ESN) [1] class.
    ...
    [1] Jaeger H. (2001a) The "echo state" approach to analysing and training recurrent neural networks. GMD Report 148,
    ...
    def _Update(self, inputs):
        ...
        self.state = self.NLfun(
            np.dot(inputs, self.I2R)
            + np.dot(self.state, self.R2R)
            + self.B2R
            + self.statenoise * np.random.randn(1, self.NN))
    ...
    def _UpdateWithFeedback(self, inputs, feedback):
        ...
        self.state = self.NLfun(np.dot(inputs, self.I2R)
                                + np.dot(self.state, self.R2R)
                                + np.dot(feedback, self.O2R)
                                + self.B2R
                                + self.statenoise * np.random.randn(1, self.NN))
