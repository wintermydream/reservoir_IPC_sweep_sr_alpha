...
def legendre_step(n, x, nm1, nm2):
    ...
    if n == 0:
        next_legendre_poly = np.ones_like(x)
    elif n == 1:
        next_legendre_poly = x.copy()
    else:
        next_legendre_poly = ((2.0 * n - 1.0) * x * nm1 - (n - 1.0) * nm2) / float(n)

    return [next_legendre_poly, nm1]
...
def legendre_incremental(n, x):
    ...
    if n == 0:
        ...
        current_legendre_poly = x.copy()
        previous_legendre_poly = np.ones_like(x)

        for index in range(int(n) - 1):
            [current_legendre_poly, previous_legendre_poly] = legendre_step(index + 2, x, current_legendre_poly, previous_legendre_poly)

    return current_legendre_poly
...
def cov_capacity(states, target, return_all_scores=False, R_inv=None):  # ,zm_target=True,zm_states=True):
    """ Compute the non-linear memory capacity based on correlations (V2: 支持多列 target 批量矢量化) """
    # states: (T, N), target: (T, M)
    score = np.zeros(target.shape[1])
    # 优化：R_inv 只算一次，且支持批量
    if R_inv is None or (isinstance(R_inv, np.ndarray) and R_inv.size == 0):
    ...
    # 批量矢量化
    P = np.dot(states.T, target) / states.shape[0]  # (N, M)
    temp = np.dot(R_inv, P)  # (N, M)
    # 每列 target 的 score
    numerators = np.sum(P * temp, axis=0)
    denominators = np.mean(target * target, axis=0)
    score = numerators / denominators
...
class CapacityIterator():
        ...
        if self.verbosity >= -1:
            tag = 'Total capacity=%(score).3f (%(perc).2f percent)' % {'score': self.totalscore(),
                                                                       'perc': 100.0 * self.totalscore() /
                                                                               estates.shape[1]}
            print(tag)
        return self.totalscore(), self.iterator_states, self.bases, self.dimensions
    ...
    def score(self, score, threshold):
        """
        Score the current state of the iterator/the current target function reconstruction.

        This method scores the current state of the iterator based on the provided score and threshold values.
        If the leaky score is greater than or equal to the threshold, then the cumulative score is updated and a tag
        is added to the list of tags. The method also updates the leaky score and current score values.

        Parameters
        ----------
        score : float
            The score for the current state of the iterator.
        threshold : float
            The threshold value used to determine if the cumulative score should be updated.
        """
        ...
        if self.leaky_score >= threshold:
            self.cumscore += score
            if self.profile:
                self.iterator_states.append({'degree': copy.deepcopy(self.degree),
    ...
    def next(self):
        """ Updates the iterator to the next state and returns a boolean value indicating whether the iterator has
        reached its end. """
        ...
        self.holding = 0
        if self.update_window_position():
            return True
    ...
    def update_delay(self):
        """
        Update the delay parameter of the iterator if necessary.
  
        Returns
        -------
        bool
            True if the delay parameter was updated, False otherwise.
        """
    ...
    def update_window_position(self):
        """
        Update the window position parameter of the iterator if necessary.

        Returns
        -------
        bool
            True if the window position parameter was updated, False otherwise.
        """
    ...
    def update_window(self):
        """
        Update the window length parameter of the iterator if necessary.

        Returns
        -------
        bool
            True if the window length parameter was updated, False otherwise.
        """
    ...
    def update_powerlist(self):
        """
        Update the power list parameter of the iterator if necessary.

        Returns
        -------
        bool
            True if the power list parameter was updated, False otherwise.
        """
    ...
    def update_variables(self):
        """
        Update the number of variables parameter of the iterator if necessary
    ...
    def update_degree(self):
        """
        Update the degree parameter of the iterator if necessary

        Returns
        -------
        bool
            True if the degree parameter was updated, False otherwise.
        """
    ...
    def __nextpos(self):
        """
        Compute the next window positions for the iterator.

        Returns
        -------
        ndarray
            The next window position for the iterator.
        """
    ...
    def __nextpowers(self):
        """
        Compute the next power list for the iterator.
        
        Returns
        -------
        ndarray
            The next power list for the iterator.
        """
...
class CapacityIteratorV2(CapacityIterator):
    def collect(self, inputs, estates):
        self.reset()
        estates = estates - np.mean(estates, axis=0)
        estates = estates / np.std(estates, axis=0)
        self.dimensions = estates.shape[1]
        if self.dimensions > 1:
            covmat = np.dot(estates.T, estates) / estates.shape[0]
            self.corrmat, crank = scipy.linalg.pinv(covmat, return_rank=True, cond=self.corr_cond)
        ...
        # 枚举所有 degree/delay 组合（可扩展更多参数）
        param_combos = []
        for degree in range(self.mindeg, self.maxdeg + 1):
            for delay in range(self.mindel, self.maxdel + 1):
                # 这里只做单变量、单 window、单 powerlist 示例
                variables = self.minvars
                window = variables
                positions = np.arange(variables)
                powerlist = np.ones(variables) * degree
                param_combos.append((degree, delay, variables, window, positions, powerlist))

        # 批量生成所有 target
        all_targets = []
        for degree, delay, variables, window, positions, powerlist in param_combos:
        ...
        targets = np.concatenate(all_targets, axis=1)  # (T, M)

        # 批量相关性计算
        scores = cov_capacity(estates, targets, return_all_scores=True)[1]  # (M,)

        # 结果还原为 all_capacities 列表
        all_capacities = []
        for i, (degree, delay, variables, window, positions, powerlist) in enumerate(param_combos):
        ...
        total_capacity = np.sum(scores)
        return total_capacity, all_capacities, len(all_capacities), self.dimensions