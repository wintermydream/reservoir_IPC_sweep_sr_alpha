...
def main() -> None:
    ...
    out_npz = out_dir / f"d1_d3_d5_capacity_vs_sr_N{args.num_nodes}_al{args.alpha:.3f}_om{args.omega:.3f}.npz"
    ...
    if out_npz.exists():
        ...
        for sr in sr_values:
            ...
            print(f"sr={sr:.3f}: d1={d1_score:.3f}, d3={d3_score:.3f}, d5={d5_score:.3f} (elapsed {elapsed:.2f}s)")
    ...
    plt.ylabel('Capacity')
    plt.title('Degree-1, Degree-3, and Degree-5 capacity vs spectral radius')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
